...
def pulp_solve_sorted(target_composition, molar_mass = pd.read_csv(os.path.join(current_dir, '..', 'Database', 'Salt Molar mass.csv')), prime=0.8, total_volume=2.5, tolerance=10E-6):
    df=pd.read_csv(inventory_directory)
...
def pulp_solve(target_composition, df=pd.read_csv(inventory_directory), molar_mass = pd.read_csv(os.path.join(current_dir, '..', 'Database', 'Salt Molar mass.csv')), prime=0.8, total_volume=2.5, tolerance=10E-6):
    ...
    amount_vars = [pulp.LpVariable(f"bottle_{i}_amount", lowBound=0, upBound=constraints["Upper_bounds"][i]) for i in range(len(df))]
